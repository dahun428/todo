			...
			<form method="post">
	 			<div class="modal-content">
	     						...
	     						<div class="card">
	     							<div class="card-body">
	     								<div class="form-group">
	     									<label>이름</label>
	     									<input type="text" class="form-control" name="name" id="signin-user-name" />
	     								</div>
	     								<div class="form-group">
	     									<label>아이디</label>
	     									<input type="text" class="form-control" name="id" id="signin-user-id"/>
	     								</div>
	     								<div class="form-group">
	     									<label>비밀번호</label>
	     									<input type="password" class="form-control" name="pwd" id="signin-user-pwd"/>
	     								</div>
	     								<div class="form-group">
	     									<label>이메일</label>
	     									<input type="text" class="form-control" name="email" id="signin-user-email"/>
	     								</div>
	     							</div>
	     						</div>
	     					</div>
	   				...
	   				<div class="modal-footer">
	     				<button type="button" id="signIn-btn" class="btn btn-outline-success btn-sm" onclick="registerUser()">회원가입</button>
		...
		<div class="modal-dialog">
			<form method="post" action="login.hta">
	 			<div class="modal-content">
	   				...
	   				<div class="modal-footer">
	     				<button type="button" onclick="loginUser()" class="btn btn-outline-success btn-sm">로그인</button>
	     				<button type="button" class="btn btn-outline-dark btn-sm" data-dismiss="modal">닫기</button>
	   				</div>
	 			</div>
 			</form>
		...
		function loginUser(){
			...
			$.ajax({
				...
				success:function(data){
					if(data.result){
						document.getElementById('success-msg-row-login').style.display = 'block';
						document.getElementById('fail-msg-row-login').style.display = 'none';
						alert( userId + '님 방문을 환영합니다.');
						location.href = 'home.hta';
					} else {
						document.getElementById('success-msg-row-login').style.display = 'none';
						document.getElementById('fail-msg-row-login').style.display = 'block';
					}
				}
			});
		}
